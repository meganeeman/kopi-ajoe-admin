-- Atomically approves one stock request and adds its quantity to the cart.
-- Factory inventory deduction intentionally remains disabled for now.
...
begin
    ...
    if v_role = 'admin'
       and coalesce(v_user_branch, 'ALL') <> 'ALL'
       and v_user_branch <> v_request_branch then
        return jsonb_build_object('success', false, 'message', 'Akses cabang ditolak.');
    end if;

    -- Serializes updates for this cart/product pair, including the first insert.
